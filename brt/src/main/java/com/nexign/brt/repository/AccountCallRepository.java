...
public interface AccountCallRepository extends CrudRepository<AccountCall, Long> {
    ...
    @Query(value = "select sum(EXTRACT(epoch FROM duration) / 60) from account_call where account_id = ?1 and date_part('MONTH', start_date) = ?2 and date_part('YEAR', start_date) = ?3", nativeQuery = true)
